package com.safetrust.keywords;
...
import com.safetrust.helpers.SystemsHelper;
...
public class WebUI {
    ...
    public static void verifyElementVisible(By by) {
        ...
        if(checkElementExist(by)){
            ...
            if(checkElementVisible(by))
                Assert.assertTrue(DriverManager.getDriver().findElement(by).isDisplayed(), "Element  visible.");
            else
                Assert.assertTrue(DriverManager.getDriver().findElement(by).isDisplayed(), "Element not visible.");
        }else
            Assert.assertTrue(false, by+" Element not Exist.");
    ...
    public static boolean checkElementVisible(By by) {
        ...
        try {
            isElementVisible = DriverManager.getDriver().findElement(by).isDisplayed();
            String txtTagName = getElementText(by);
//            System.out.println("Verify----------------- " + by + " is displayed " + isElementVisible + " name : " + txtTagName);
        }catch (Exception e){
            return false;
        }
    ...
    public static boolean checkElementVisible(By by, String text) {
        boolean isElementVisible = false;
        try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(10));
            wait.until(ExpectedConditions.visibilityOfElementLocated(by));
            String txtTagName = getElementText(by);
             isElementVisible = DriverManager.getDriver().findElement(by).isDisplayed() & txtTagName.contains(text);
    ...
    public static void verifyElementVisible(By by, String text, String msg) {
        try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(10));
            if(checkElementExist(by)){
                wait.until(ExpectedConditions.visibilityOfElementLocated(by));
                String txtTagName = getElementText(by);
                boolean isElementVisible = DriverManager.getDriver().findElement(by).isDisplayed() & txtTagName.contains(text);
                if(isElementVisible)
                    Assert.assertTrue(isElementVisible, msg);
                else
                    Assert.assertFalse(isElementVisible, msg);
            }
                else
    ...
    public static void verifyElementVisible(By by, String message) {
        try {
            ...
            if(checkElementExist(by)){
                wait.until(ExpectedConditions.visibilityOfElementLocated(by));
                System.out.println("Verify " + by + " is displayed");
                boolean isElementVisible = DriverManager.getDriver().findElement(by).isDisplayed();
                if(isElementVisible)
                    Assert.assertTrue(isElementVisible, message);
                else
                    Assert.assertFalse(isElementVisible, message);
            }
            else
    ...
    public static void openURL(String url) {
        ...
        if (PropertiesHelper.getValue("SCREENSHOT_STEP").equals("yes")) {
            CaptureHelper.takeScreenshot("openURL_" + SystemsHelper.makeSlug(url));
        }
    }
    ...
    public static void clickElement(By by) {
        ...
        if (PropertiesHelper.getValue("SCREENSHOT_STEP").equals("yes")) {
            CaptureHelper.takeScreenshot("clickElement_" + SystemsHelper.makeSlug(by.toString()));
        }
    }
    ...
    public static void setText(By by, String value) {
        ...
        if (PropertiesHelper.getValue("SCREENSHOT_STEP").equals("yes")) {
            CaptureHelper.takeScreenshot("setText_" + SystemsHelper.makeSlug(by.toString()));
        }
    }
    ...
    public static void scrollToElement(WebElement element) {
        ...
        if (PropertiesHelper.getValue("SCREENSHOT_STEP").equals("yes")) {
            CaptureHelper.takeScreenshot("scrollToElement_" + SystemsHelper.makeSlug(element.getText()));
        }
    }
